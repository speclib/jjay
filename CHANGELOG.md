...
- **Unit tests for `internal/cleanup`** — the only command package that had no test file now has one. Covers `removeDirectory` (removes an existing dir under a temp `WorkspaceRoot`, tolerates a missing one), the pure `tmuxTarget` helper (`window` vs `session:window`), and the tolerance branches of `killWindow`/`forgetWorkspace`/`Cleanup` — all asserted as tolerance (no error, no panic) so they pass with or without a live tmux server or jj repo. Raises `internal/cleanup` coverage from 0% to 59.3%. Test-only; no production code changed. See [proposal](openspec/changes/archive/2026-06-05-add-cleanup-unit-tests/proposal.md).
