...
Defines how jjay resolves per-agent command templates (`launch`/`resume`) from a layered configuration, how built-in defaults are materialized on init, and how jjay's configuration relates to openspec's configuration.
