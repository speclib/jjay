...
The `init` capability prepares an arbitrary project for orchestration by jjay: it initializes openspec, installs the jjay Claude integration and skill, writes an `AGENTS.md`, and optionally sets up jj and hooks. It delegates to canonical initializers, is idempotent and non-destructive, and supports non-interactive use.
