...
- **Work frontier, not `<change>@` (ADR-013).** Define `WORK = ancestors(<change>@) & main.. & ~empty()`. This includes `@` and `@-` (and any ancestor of `@`). The rebase and merge operate over this frontier. **Spike result:** a true divergent *sibling* (a commit `@` never descended from) is NOT reachable from `<change>@` — jj associates exactly one `@` per workspace, so there is no workspace-scoped handle to an orphan `@` never visited. Instance 3 is therefore handled by *detection* (smoke test fails loud, workspace kept), not auto-inclusion. Op-log sibling discovery was considered and rejected as fragile (non-goal).
...
- **Smoke test L1+L2 (rse4).** Capture the changed-file set across `WORK` before merge via `jj diff --summary`, keeping only **A/M** files (net-deleted files excluded). After merge: L1 — if the workspace had work but main gained nothing → FAIL. L2 — every captured file must be present on main, matched by exact path **or basename** so a merge-time move/rename (esp. `/opsx:archive` relocating `changes/X/` → `changes/archive/<date>-X/`) is not a false positive → FAIL only on a genuine miss. Verbose by default (pre-1.0); failure message lists missing files + the op id.
...
- **Test mirrors the q6ko instances.** Add an integration test per reproduced/handled instance: empty-`@`-work-in-`@-` (`TestMerge_EmptyAtWorkInParent`), forget-on-success/no-staleness (`TestMerge_NotStaleAfterMerge`), the L1/L2 gate (`TestSmokeTest_L1L2`), reachable-work lands (`TestMerge_SmokeDetectsMissingFile`), and the archive-move false-positive guard (`TestSmokeTest_ToleratesArchiveMove`). Helper `workspaceExists()` asserts forget-on-success / keep-on-failure (cleaner than the originally-planned `isStale` since forget makes the workspace *gone*, not stale). The true-orphan sibling is undetectable (jjay-ychu) so no auto-include test exists for it.
...
- **L2 false positives (CONFIRMED in the wild, fixed).** A file legitimately removed or *moved* trips exact-path "missing on main". This actually happened: merging a change that ran `/opsx:archive` moved `openspec/changes/X/tasks.md` → `changes/archive/<date>-X/tasks.md`, and exact-path L2 falsely failed while the work was on main. Fix: capture only **added/modified** files (drop net-deletes), and match L2 presence by exact path **or basename** (move/archive tolerant). Regression test `TestSmokeTest_ToleratesArchiveMove`. Residual: two files sharing a basename could mask one miss — accepted pre-1.0; L3 content verification is the follow-up.
